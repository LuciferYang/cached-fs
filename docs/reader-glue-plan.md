...
> **Status:** draft v7.5, 2026-05-25 (HEAD = 104247b). Plan-only; no code changes yet. Round-10 review fixes: (a) **`withScanId` re-entrancy hardened against Spark thread reuse**. v7.4 threw `IllegalStateException` on any non-null ThreadLocal — but on Spark executor threads, a prior task's crash between `withScanId(...)` and its `close()` leaves the slot stale; the next reusing task then fails for a defect introduced by its predecessor. v7.5 changes semantics: a second `withScanId` on a thread with a stale slot logs WARN ("stale scanId slot detected; recovering — likely a prior task crashed between withScanId and close"), calls `removeScanTracker(stale)` to evict the prior tracker, and proceeds with the new scanId. Restores benign-leak property of the dangling-tracker design. (b) **`prefetchEligibleSuppressedBytes` operator guidance**: v7.4 prescribed correlating with "per-scan readPct distribution" which the plan didn't expose. v7.5 replaces with two observable signals: (i) ratio `prefetchEligibleSuppressedBytes / readBytes` against a known-baseline workload, (ii) correlation with `scanTrackerMaxEntries()` growth — per-scan distribution dropped from guidance (open follow-up). (c) **`prefetchBudgetShed` / `prefetchHeapShed` reconciled**: bump-site comment in §step 4 referenced these but the canonical Phase 5c counter list didn't enumerate them. v7.5 re-routes the bump through the existing `incPrefetchSkipped(reason)` counter with reason strings "budget" and "heap_pressure". (d) **`withLongFunctionGauge` signature corrected** to `ToLongFunction<String>` (was incorrectly written as `LongSupplier`). (e) **Orphan-mutation perf cost quantified**: integrator recommendation to close streams before `removeScanTracker`; otherwise wasted ~8 MB/s young-gen churn per orphan-stream. (f) **Test-quality cleanups**: `ScanTrackerNeverRemovedTest` now uses two back-to-back gauge reads instead of Thread.sleep(100); nested-withScanId error message tightened to remove misleading "use distinct scanIds" clause.
...
2. **`CacheBootstrap.currentScanId()` + `withScanId(String)`** — `ThreadLocal<String>` accessor plus `AutoCloseable withScanId(scanId)` that on `close()` (a) calls `threadLocal.remove()` AND (b) calls `removeScanTracker(scanId)` to evict the tracker entry from `bootstrap.scanTrackers` so the inner `TrackingData` map (potentially MB-scale on partitioned scans) is GC-eligible. Without this, long-lived JVMs (Spark driver, YARN NM with the FileSystem cache) accumulate trackers indefinitely.
   ...
   **Stale-slot recovery, NOT nested-rejection.** On entry, `withScanId` checks `threadLocal.get()`. The handling depends on the cause:
   ...
   - **Stale slot from a crashed prior task (common on Spark executor thread reuse):** the prior task entered `withScanId(...)` and crashed before `close()` ran. v7.5 logs WARN ("stale scanId slot 'A' detected; auto-recovering — likely prior task crashed between withScanId and close"), calls `removeScanTracker(staleScanId)` to evict the orphaned tracker, then sets the slot to the new scanId. This **restores the benign-leak property of the dangling-tracker design** and prevents Spark thread-reuse cascade failures. A `IoStatistics.staleScanIdRecoveries` counter ticks each recovery so operators can detect crashing-task patterns.
...
- **Stale-slot recovery (NOT rejection)**: `bootstrap.withScanId("A")` (no close, simulate a prior task that crashed); on the same thread call `bootstrap.withScanId("B")`. Assert: no exception; WARN log emitted with a "stale scanId slot" message; `bootstrap.scanTrackerEntries()` reflects only "B"'s tracker (the "A" tracker was auto-removed); `IoStatistics.staleScanIdRecoveries()` incremented by 1. The "B" close then evicts B normally.
   ...
   **Bump-site for `prefetchEligibleSuppressedBytes`** (the lower-tail observability counter from §Decisions §6):
   ...
   if (pendingPrefetch == null
     ...
     if (readPct >= prefetchPctThreshold && admissionGate()) {
       ...
       // admissionGate-false: bump prefetchSkipped(reason) instead — see counter list.
       ioStats.incPrefetchSkipped(admissionGate.reason());  // "budget" | "heap_pressure"
     }
   }
   ```
   The position guard, backoff check, and `pendingPrefetch != null` guard all happen BEFORE the bump branch — so the counter measures "density predicate suppressed a would-be prefetch" exclusively. `admissionGate()` failures (byte budget / heap pressure) deliberately do NOT bump `prefetchEligibleSuppressedBytes`; they bump `incPrefetchSkipped(reason)` with reason strings `"budget"` or `"heap_pressure"`. **The `AdmissionGate` helper returns a small typed result `record AdmissionResult(boolean admit, String reason)` so the caller can write the correct reason** — `reason()` is `null`/empty on `admit=true`.
...
Phase 5a also adds one counter:
- `staleScanIdRecoveries()` — bumped when `withScanId` enters with a stale ThreadLocal slot and auto-recovers (see Phase 5a wiring §2). Surfaces crashing-task patterns where prior tasks fail before `close()`.

All four are byte-only (no event count partner) for the prefetch counters since their use cases are about volume; `staleScanIdRecoveries` is event-count-only (no bytes meaningful). **Open follow-up**: add event-count partners for the three prefetch counters if operators need alerting rate-of-suppression-events rather than rate-of-suppressed-bytes; mirrors the existing pattern in other IoStatistics pairs. Adapter exposes under `cachedfs_stream_prefetch_skipped_bytes`, `cachedfs_stream_prefetch_evicted_bytes`, `cachedfs_stream_prefetch_eligible_suppressed_bytes`, and `cachedfs_stale_scan_id_recoveries` respectively. Implementation order: the IoStatistics fields and inc* methods land alongside Phase 5c.0 (prefetch counters) and Phase 5a wiring (staleScanIdRecoveries); `AggregatedIoStatistics.add(IoStatistics)` (introduced in 5a wiring step 7) must include all four in its snapshot loop — verified by an acceptance test that constructs an AggregatedIoStatistics from a stream whose suppressed counter was bumped and asserts the aggregate reflects it. **Bump-site invariant for future contributors**: every admission-rejection path MUST bump a dedicated `IoStatistics` counter (a known `prefetchSkipped(reason)` bucket or a new field); a new admission failure mode that falls through silently (or wrongly into `prefetchEligibleSuppressedBytes`) is a design-invariant violation. Acceptance test asserts the sum of all rejection counters over a saturated run equals the count of admission-gate-false branches.
