...
> **Status:** draft v7.4, 2026-05-25 (HEAD = 1a2423a). Plan-only; no code changes yet. Round-9 review fixes: (a) **bump-site for `prefetchEligibleSuppressedBytes`** explicitly placed in the §5c admission-gate else branch (after the position-tail guard succeeds AND density predicate fails AND backoff has elapsed) so the counter never over-counts non-eligible chunks. (b) **TaskCompletionListener guidance for `removeScanTracker`** added: unlike `withScanId` (ThreadLocal-bound), `removeScanTracker` is thread-safe and IS the right TCL payload for Configuration-key integrators. (c) **Nested `withScanId(scanId)` re-entrancy**: now documented as forbidden; second nested call with the same scanId throws `IllegalStateException` rather than silently double-allocating on inner-close eviction. (d) **`ScanTrackerNeverRemovedTest` concrete acceptance design** added to the Phase 5a wiring acceptance-tests list. (e) **Counter conflation documented** as a union signal in the prefetchEligibleSuppressedBytes spec; correlated with `scanTrackerMaxEntries` and per-scan readPct distribution for disambiguation. (f) **`prefetchEligibleSuppressedBytes` re-located** to Phase 5c's new-counter list (was previously orphaned in §Decisions). (g) **Dangling-tracker / concurrent-remove race** documented as benign in Phase 5a wiring. (h) **Within-package mutability** explicitly called out — only PrefetchTask and DiscardAndCountHandler are legitimate writers of PENDING_VH / lastRejectionNanos / clearPendingPrefetchIf. (i) **IOStatisticsBinding location** disambiguated to `org.apache.hadoop.fs.statistics.impl.IOStatisticsBinding` (HADOOP-17450, hadoop-common 3.3.1+). (j) **removeScanTracker idempotency** stated. (k) Constructor-time nanoTime() underflow edge documented as acceptable risk.
...
6. **TrackingId scoping.** Phase 5a-prework shrinks `TrackingId`'s streamKind from 5 bits to 2 bits (only one streamKind value is used at the Hadoop layer), gaining 3 bits → **29-bit node space** (~536M buckets). This is the v7 change relative to v6's documented 26-bit node. Plan wires `TrackingId.of(fileNumNode, 0)` where:
   ...
   **Mitigation (Phase 5c, not 5a):** Phase 5c adds an `IoStatistics.prefetchEligibleSuppressedBytes` counter that ticks when the admission gate's density predicate is the sole reason a chunk is dropped (i.e., `pendingPrefetch == null` AND `position in trigger-tail-fraction` AND `backoff elapsed` AND `readPct < threshold`). See §Phase 5c step 4 for the exact bump site. The counter is a **union signal**: it ticks for (a) genuine low-density streams correctly suppressed and (b) collision-induced false-negative suppression on hot files. Operators cannot disambiguate from this counter alone; they should correlate with `scanTrackerMaxEntries()` (high = more collision pressure) and the per-scan readPct distribution to attribute the suppression. A future per-tracker collision-counter would refine this; tracked as an open follow-up.
...
2. **`CacheBootstrap.currentScanId()` + `withScanId(String)`** — `ThreadLocal<String>` accessor plus `AutoCloseable withScanId(scanId)` that on `close()` (a) calls `threadLocal.remove()` AND (b) calls `removeScanTracker(scanId)` to evict the tracker entry from `bootstrap.scanTrackers` so the inner `TrackingData` map (potentially MB-scale on partitioned scans) is GC-eligible. Without this, long-lived JVMs (Spark driver, YARN NM with the FileSystem cache) accumulate trackers indefinitely.
   ...
   **Re-entrancy.** Nested `withScanId(scanId)` on the same scanId is **forbidden** — `withScanId` checks `threadLocal.get() != null` on entry and throws `IllegalStateException("nested withScanId with same/different scanId is not supported; use distinct scanIds or release the outer scope first")` if a scope is already active. This prevents the otherwise-silent double-allocation that would occur when the inner `close()` evicts the tracker while the outer scope is still mutating it. Integrators with nested-scan patterns must produce distinct scanIds per nesting level (e.g., `parent.task-7.batch-3`).
   ...
   **Dangling-tracker / concurrent-remove race.** Both are benign:
   - **Orphan-mutation:** an in-flight `CachingInputStream` opened with scanId X retains a strong reference to the now-evicted ScanTracker after `withScanId(X).close()`. Its continued `recordReference`/`recordRead` calls mutate the orphan tracker; mutations are simply ignored by readers (admission gate, gauge, future `trackerFor` calls) and the tracker becomes GC-eligible after the stream closes. No leak, no inconsistency.
   - **Concurrent-remove vs trackerFor:** if thread A's `removeScanTracker(X)` interleaves with thread B's `computeIfAbsent` inside `trackerFor(X)`, B may create a freshly-allocated ScanTracker that immediately becomes an orphan from A's perspective. Same benign properties as above.
...
- **ScanTrackerNeverRemoved (observability-only)**: `bootstrap.trackerFor("orphan")` then assert `scanTrackerEntries() == 1`; without any close or remove, sleep 100 ms and re-assert `== 1` (proving the leak is detectable but not auto-recovered); then `bootstrap.removeScanTracker("orphan")` and assert `== 0` (proving the explicit remove path works). Test verifies the gauge surfaces the failure mode — it does not guarantee runtime leak prevention.
- **Nested withScanId rejected**: `bootstrap.withScanId("A")` (no close), then a second `bootstrap.withScanId("A")` throws `IllegalStateException`; a second `withScanId("B")` from the same thread also throws (cannot stack scopes on one thread). Assert the message mentions "nested withScanId".
...
4. **Consumer thread, post-chunk-N read**: if `pendingPrefetch == null` AND `position >= chunkNEnd - (loadQuantum * triggerTailFraction)` AND `tracker.data(trackingId).readPct() >= prefetchPctThreshold` AND `admissionGate()` AND `System.nanoTime() - lastRejectionNanos >= REJECTION_BACKOFF_NS` (per-stream backoff, default 100 ms via `fs.cached.prefetch.rejection-backoff-ms`): submit the prefetch task via `prefetchExecutor.execute(task)`. The backoff prevents a hot resubmit loop under sustained queue saturation: after a rejection the stream waits 100 ms before re-attempting prefetch submission, while the consumer's normal per-chunk path proceeds at its native rate.
   ...
   **Bump-site for `prefetchEligibleSuppressedBytes`** (the lower-tail observability counter from §Decisions §6):
   ...
   if (pendingPrefetch == null
     ...
     // The chunk is position-eligible AND not in backoff: the density predicate
     // is the deciding gate. Bump only when density is the sole reason we drop.
     double readPct = tracker.data(trackingId).readPct();
     if (readPct >= prefetchPctThreshold && admissionGate()) {
     ...
     // admissionGate-rejected does NOT bump; that's a budget-shed signal, distinct.
   }
   ```
   The position guard, backoff check, and `pendingPrefetch != null` guard all happen BEFORE the bump branch — so the counter measures "density predicate suppressed a would-be prefetch" exclusively. `admissionGate()` failures (byte budget / heap pressure) deliberately do NOT bump this counter (they have separate `prefetchBudgetShed` / `prefetchHeapShed` counters in the same Phase 5c counter set).
...
- `prefetchSkipped(reason)` — rejection-handler bump for queue-full and other RejectedExecutionException reasons.
- `prefetchEvictedBeforeUse()` — TTL evicted the prefetched entry before the consumer awaited.
- `prefetchEligibleSuppressedBytes()` — admission gate's density predicate rejected a position-eligible, backoff-elapsed chunk; lower-tail observability for collision-induced suppression (see §Decisions §6). **Union signal**: bumps on both genuine cold-scan suppression and collision-induced false-negative suppression; not disambiguable from this counter alone — operators correlate with `scanTrackerMaxEntries()` and per-scan readPct distribution.

All three are byte-only (no event count partner) since the use cases are about volume. Adapter exposes under `cachedfs_stream_prefetch_skipped_bytes`, `cachedfs_stream_prefetch_evicted_bytes`, and `cachedfs_stream_prefetch_eligible_suppressed_bytes` respectively. Implementation order: the IoStatistics fields and inc* methods land alongside Phase 5c.0; `AggregatedIoStatistics.add(IoStatistics)` (introduced in 5a wiring step 7) must include them in its snapshot loop — verified by an acceptance test that constructs an AggregatedIoStatistics from a stream whose suppressed counter was bumped and asserts the aggregate reflects it.
