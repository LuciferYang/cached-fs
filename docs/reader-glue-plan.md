...
> **Status:** draft v7.3, 2026-05-25 (HEAD = 98fb920). Plan-only; no code changes yet. Round-8 review fixes: (a) **scanTrackers eviction trigger**: `withScanId(scanId)` AutoCloseable now removes the tracker entry on close, and `bootstrap.trackerFor(scanId)`'s contract states "tracker lives until matching `withScanId` close OR explicit `bootstrap.removeScanTracker(scanId)`"; unbounded growth on long-lived JVMs is now bounded by integrator discipline + the open-follow-up LRU as a defense-in-depth net. (b) **staleDensity test asymmetry**: explicitly documented as bounding only the upper tail (wasted prefetch); collision-induced missed-prefetch is now a documented known risk with an additional one-line `prefetchEligibleSuppressedBytes` IoStatistics counter to make the lower tail observable. (c) **sibling vs nested class rationale**: design note added — sibling top-level package-private classes chosen so `DiscardAndCountHandlerTest` and `PrefetchTaskTest` can exercise them in isolation without instantiating a full `CachingInputStream`; the wider package-private surface is contained to one package. (d) **IoStatisticsAdapter dynamic gauge mechanism specified**: adapter implements `IOStatistics` directly with a `gauges()` view that calls `bootstrap.scanTrackerEntries()` at read time (live view, not snapshot). (e) **scanTrackerEntries single-sum gauge gap**: add a sibling `scanTrackerMaxEntries()` accessor so the per-tracker LRU trigger condition is observable as `max`, not just `sum`. (f) **`Long.MIN_VALUE/2` overflow defense**: lastRejectionNanos and heapPressureLastCheckedNs now initialize in the constructor as `System.nanoTime() - HEAP_PRESSURE_TTL_NS - 1` (resp. `... - REJECTION_BACKOFF_NS - 1`), eliminating the 146-year edge. (g) **Decrement integrity-check failure mode** documented: throws escape to executor's uncaught-handler; counter left inconsistent; bug-only condition. (h) **Collision math 10k case** corrected to ~9% (was rounded to 11%).
...
6. **TrackingId scoping.** Phase 5a-prework shrinks `TrackingId`'s streamKind from 5 bits to 2 bits (only one streamKind value is used at the Hadoop layer), gaining 3 bits → **29-bit node space** (~536M buckets). This is the v7 change relative to v6's documented 26-bit node. Plan wires `TrackingId.of(fileNumNode, 0)` where:
   ...
   **Lower tail (missed prefetch) is NOT bounded by the existing test.** For the hot member of a collision pair, mixed density is below true density — prefetch may be suppressed below the threshold and the hot file loses its prefetch acceleration silently. At 100k files (~10 collisions, ~10 hot files possibly affected) and at 500k files (~233 collisions, ~233 hot files possibly affected), this is bounded in count but not in performance impact. **Mitigation:** Phase 5a adds an `IoStatistics.prefetchEligibleSuppressedBytes` counter that ticks whenever the admission gate's density predicate rejects a chunk that would otherwise be a prefetch candidate (i.e., `readPct < threshold` AND `position is in trigger-tail-fraction`). Operators can watch this counter relative to `prefetchBytes` to detect when collision-induced suppression becomes a significant fraction of would-be prefetches. The metric does not bound the impact but makes it observable.
...
- **`ScanTracker.size()` gauge.** Add `public int size() { return data.size(); }` (O(1) approx on `ConcurrentHashMap` — non-atomic but lock-free; acceptable for a gauge per Hadoop/JMX/Prometheus semantics). Implementation note: this method depends on the new `ConcurrentMap<TrackingId, AtomicReference<TrackingData>> data` field introduced earlier in this prework; the gauge cannot ship before the data refactor lands.
  ...
  Both are exposed via `IoStatisticsAdapter`'s `gauges()` map as **live-view suppliers** (not a snapshot map). The adapter implements `org.apache.hadoop.fs.statistics.IOStatistics` and returns a `Map<String, Long>` from `gauges()` whose `get(...)` delegates to `bootstrap.scanTrackerEntries()` / `scanTrackerMaxEntries()` on each call — built on Hadoop's `IOStatisticsBinding.dynamicIOStatistics(...)` (hadoop-common 3.3+, present at our pinned 3.4.1).
...
2. **`CacheBootstrap.currentScanId()` + `withScanId(String)`** — `ThreadLocal<String>` accessor plus `AutoCloseable withScanId(scanId)` that on `close()` (a) calls `threadLocal.remove()` AND (b) calls `removeScanTracker(scanId)` to evict the tracker entry from `bootstrap.scanTrackers` so the inner `TrackingData` map (potentially MB-scale on partitioned scans) is GC-eligible. Without this, long-lived JVMs (Spark driver, YARN NM with the FileSystem cache) accumulate trackers indefinitely. Also adds a public `void removeScanTracker(String scanId)` for integrators that resolve scanId via the `Configuration` key (no try-with-resources path). Javadoc warns: "Single-threaded use only. Do NOT pair with Spark `TaskContext.addTaskCompletionListener` — close may run on a different thread and leak the slot. Use try-with-resources inside the task body." **Contract:** the tracker lives from first `trackerFor(scanId)` until matching `withScanId.close()` OR explicit `removeScanTracker(scanId)`; ScanTrackerNeverRemovedTest asserts that the absence of either path is detectable via the `scanTrackerEntries()` gauge (the open-follow-up LRU is a defense-in-depth net on top, not a substitute for integrator discipline).
