...
- `seqHwmRegimeResets` (v7.18, event-count, no bytes) — incremented inside the sequential read path's regime-change branch (when `|prev HWM − new sequential candidate| > 2 × chunkSize` resets HWM down to the sequential candidate). Surfaces oscillation under sustained mixed sequential+positional traffic at >2-chunk divergence. Single-writer (consumer thread) like `admissionGateFalseCount`; same package-private accessor pattern for tests. Acceptance test bound: under a workload of N=1000 contiguous sequential reads with no positional interleave, `seqHwmRegimeResets <= 1` (only the optional bootstrap-from-positional-leftover transition); under sustained mixed traffic at >2-chunk divergence, the counter ticks at the divergence-event rate, observable as a Prometheus rate gauge.
