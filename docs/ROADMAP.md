...
| M3 | `cached-fs-cli` — Picocli-driven ops tool. | `cached-fs-cli` | 🟢 Done (MVP) | Shipped `config` (load Hadoop XMLs + inline `-D`, validate `fs.cached.*` resolvers, dump effective values with defaults — exits 2 on range errors) and `version` subcommands. Live-state inspection (`inspect <path>`, `stats --window`, `drain`, `purge`) deferred to a follow-up — requires JMX/RPC infrastructure that doesn't exist yet. Micrometer/Prometheus from M1 already covers cumulative observability. |
| M3.1 | Live-state CLI subcommands — `inspect <path>`, `stats --window`, `drain`, `purge --pattern`. | `cached-fs-cli` | ⚪ Deferred | Needs a JMX MBean or RPC endpoint on `CacheBootstrap` so the CLI can query a running JVM. Defer until there's a documented ops use case the Prometheus surface can't already cover. |
