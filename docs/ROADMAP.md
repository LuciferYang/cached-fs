...
| M3 | `cached-fs-cli` — Picocli-driven ops tool. | `cached-fs-cli` | 🟢 Done | Shipped `config` + `version` (static subcommands) and live-state subcommands via the M3.1 JMX bean (see below). |
| M3.1 | Live-state CLI subcommands — `inspect <key>`, `stats`, `recent-streams`, `drain`, `purge <regex>`. | `cached-fs-hadoop` + `cached-fs-cli` | 🟢 Done | New `CachedFsBootstrapMXBean` registered on the platform MBean server at install (toggle via `fs.cached.jmx.enabled`, default true); 5 CLI subcommands connect via local platform server by default or `--jmx-url <service-url>` for remote. Destructive ops (`drain`, `purge`) require `--yes`. `stats --window <seconds>` does a client-side two-shot (snapshot, wait, snapshot) and prints per-counter change + per-second rate via the new typed `counters()` MXBean op — no background sampler in the target JVM. |
...
| R1.1 | Build a JMH benchmark harness and measure the right multiplier on a representative workload | 🟢 Done (harness) | New `cached-fs-bench` module: `PrefetchMultiplierBenchmark` (multiplier × latency × access-pattern sweep over a latency-injecting in-memory backing store) + `AdmissionGateBenchmark` (gate accounting cost under contention). Run/results/verdict in `docs/bench/prefetch-multiplier.md`. The harness is in the reactor + guarded by `BenchHarnessSmokeTest`; benchmarks run on-demand via `java -jar`, never in CI. Filling the results table on representative hardware + deciding whether to revise the default-4 is the remaining manual step. |
