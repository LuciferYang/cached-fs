...
1. **Walk + classify.** For each chunk in `[startChunk, endChunk]`, call `findOrCreate(key, size, false)`. Build a `List<Resolved>`:
   ...
   - `Waiting` → **abort sub-routine**: release every pin held so far in ascending offset order via `pin.close()` for both Hit and Exclusive pins (CachePin.close routes Exclusive pins to `CacheShard.releaseFailedExclusive` internally — see CachePin.java line 101-112; we never call the package-private `releaseFailedExclusive` directly from cached-fs-hadoop). Ascending order is **FIFO/queue discipline** (not stack/LIFO); chosen because pins are independent — each `close()` only affects its own entry's pin-count, no order dependency — and ascending matches the iteration order used to acquire them, making the unwind code straightforward to read. If any release throws, accumulate via `Throwable.addSuppressed` and continue releasing the rest; rethrow the original abort trigger only after every pin is released. Await the future. Increment a per-call `restartCount`; restart from `startChunk`. **Bound:** `fs.cached.coalesce.max-restarts` (default 3). On exceeding the bound, fall back to the per-chunk `copyChunk` path.
...
4. **Promote.** For each Exclusive in the group, `exclusiveToShared(true)`. On any throw in steps 3 or 4: every still-Exclusive in the group is released via `pin.close()` (routes to `releaseFailedExclusive` internally — see CachePin.close line 101-112); every already-promoted Shared pin and every Hit pin is closed (suppressed-exception chain); rethrow. **Partial-promotion safety:** if step 4 throws after some Exclusives have already been promoted to Shared, the already-promoted entries hold filled buffers (preadv in step 3 ran to completion before step 4 began iterating). Closing the Shared pins is correct — the entries stay populated and other readers will see correctly-filled chunks, NOT partially-filled regions. The pin-count goes to zero on each close, but the entry's buffer is fully valid. A separate IT asserts no reader observes a partially-filled chunk after a forced mid-group preadv failure.
