...
- **TrackingId bit-width change** (v7). Shrink `STREAM_KIND_BITS` from 5 to 2 in the packed representation; node range widens from 26-bit to 29-bit automatically since `NODE_MAX = 1 << (Integer.SIZE - STREAM_KIND_BITS - 1)` is derived. `TrackingId.of(int node, int streamKind)` validation tightens to `streamKind < 4` and `node < (1<<29)`. Existing call sites all use `streamKind == 0`, so the change is non-breaking for the cached-fs tree. **Also updates** the TrackingId class-level javadoc to describe the packed layout precisely: **bit 31 = reserved sign bit (zero); bits 30..2 = node (29 bits); bits 1..0 = streamKind (2 bits); packed value `= (node << 2) | streamKind` ranges `[0, 2^31)`**. (Previous javadoc loosely said "node in the high 27 bits and stream kind in the low 5"; v7 updates to the 29-bit/2-bit split.) The `NODE_MAX` derivation comment in code reads `NODE_MAX = 1 << (Integer.SIZE - STREAM_KIND_BITS - 1) = 1 << (32 - 2 - 1) = 2^29` (the `-1` reserves the sign bit; node values stay in `[0, 2^29)`). Adds a 1-line ScanTracker pre-work acceptance test asserting `TrackingId.of((1<<29)-1, 0).id() == ((1<<29)-1) << 2`.
